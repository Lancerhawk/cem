...
import { updateWorkflowTask } from '@/app/lib/actions/workflow_action'
import { broadcastTaskUpdated } from '@/lib/sse-broadcast'
...
export async function PUT(
  ...
  try {
    ...
    const result = await updateWorkflowTask(id, taskId, body)
      ...
      try {
        ...
        if (enrichedTaskResponse.success && enrichedTaskResponse.tasks) {
          ...
          if (updatedTask) {
            ...
            broadcastTaskUpdated(id, updatedTask)
            console.log(`Broadcasted enriched task update to workflow ${id}`)
          } else {
            // Fallback to basic task data
            broadcastTaskUpdated(id, result.task || { _id: taskId, ...body })
            console.log(`Broadcasted basic task update to workflow ${id}`)
          }
        } else {
          // Fallback to basic task data
          broadcastTaskUpdated(id, result.task || { _id: taskId, ...body })
