...
import { updateTaskStatus } from '@/app/lib/actions/workflow_action'
import { broadcastTaskStatusChanged } from '@/lib/sse-broadcast'
...
export async function PUT(
  ...
  try {
    ...
    const result = await updateTaskStatus(id, taskId, body)
      ...
      try {
        ...
        if (enrichedTaskResponse.success && enrichedTaskResponse.tasks) {
          ...
          if (updatedTask) {
            ...
            broadcastTaskStatusChanged(id, updatedTask)
            console.log(`Broadcasted enriched task status change to workflow ${id}`)
          } else {
            // Fallback to basic task data
            broadcastTaskStatusChanged(id, result.task || { _id: taskId, ...body })
            console.log(`Broadcasted basic task status change to workflow ${id}`)
          }
        } else {
          // Fallback to basic task data
          broadcastTaskStatusChanged(id, result.task || { _id: taskId, ...body })
